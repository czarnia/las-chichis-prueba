...
public class MapaObservable extends JPanel implements Observer{
	...
	private EstrategiaDeClick estrategiaPorDefecto;
	private EstrategiaDeClick estrategiaParticular = null;
	...
	public MapaObservable(SectorJuego sectorJuego, Partida partida, HashMap<Class<?>, Class<?>> vistas) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		this.estrategiaPorDefecto = new EstrategiaMostrarInfoSeleccionado(sectorJuego);
		...
		for (int i = 0; i < this.partida.getMapa().ancho(); i++){
			...
			for (int j = 0; j < this.partida.getMapa().alto(); j++){
				...
				vistaNueva.addMouseListener(new VistasListener(this));
				add(vistaNueva);
    ...
    public void click(MouseEvent e, Vista vista) {
    	
    	TreeMap<Integer, TreeMap<Integer, Celda>> mapaIterable = this.partida.getMapa().devolverMapaEstatico();
    	
    	Coordenada coordenadaClickeado = vista.getCoordenada();
    	
    	Celda celda = mapaIterable.get(coordenadaClickeado.fila()).get(coordenadaClickeado.columna());
		Terreno terrenoInferior = celda.getCapaInferior();
		Terreno terrenoSuperior = celda.getCapaSuperior();
		
		if(terrenoInferior.estaOcupado() && terrenoSuperior.estaOcupado()){
			
			//Mostrar Cartel de Eleccion:
			
			JPopupMenu cartelEleccion = new JPopupMenu();
			
			JMenuItem abajo = new JMenuItem("Abajo");
			abajo.addActionListener(new EleccionListener(abajo));
		    cartelEleccion.add(abajo);
		    JMenuItem arriba = new JMenuItem("Arriba");
		    arriba.addActionListener(new EleccionListener(arriba));
		    cartelEleccion.add(arriba);
		    
		    cartelEleccion.show(e.getComponent(), e.getX(), e.getY());
		   	//WAIT NOTIFY???
		    Ubicable ubicable = null;
		    if(arriba.isSelected()){
		    	new MensajeDeError("Debug: Arriba");
		    	ubicable = terrenoSuperior.getUbicableEnTerreno();
		    }else if(abajo.isSelected()){
		    	new MensajeDeError("Debug: Abajo");
		    	ubicable = terrenoInferior.getUbicableEnTerreno();
		    }
			
		}

    	if(this.estrategiaParticular != null){
    		
    		this.estrategiaParticular.seHizoClick(vista);
    	
    	}
    	else{
    	
    		this.estrategiaPorDefecto.seHizoClick(vista);
    	
    	}
    	
    }
    
    static class EleccionListener implements ActionListener{
    	
    	JMenuItem item;

    	public EleccionListener(JMenuItem item){
    ...
    static class VistasListener implements MouseListener{
    	
    	private MapaObservable mapa;
    	
    	public VistasListener(MapaObservable mapa){
    	...
    	public void mouseClicked(MouseEvent click) {
    		...
    		Vista vista = (Vista) click.getSource();
    		mapa.click(click, vista);
